...
[
  {
    ...
    generateScenarioDescriptions: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        const prompt = `你是一位专业的商业调研分析师。请基于以下信息，为三个研究场景生成有针对性的、与用户输入紧密相关的描述：

**目标人群**：${input.targetAudience}
**调研目标**：${input.researchGoal}

重要提示：
- 场景描述必须与用户输入的"目标人群"和"调研目标"直接相关
- 不要使用通用的默认描述
- 描述应该具体、有针对性，反映真实的使用场景

请为以下三个场景生成简洁的描述（20-30字）：
1. **工作场景**：描述该目标人群在工作中与调研目标相关的具体场景
2. **个人/家庭场景**：描述该目标人群在个人生活中与调研目标相关的具体场景
3. **两个场景都关注**：描述综合研究两个场景对实现调研目标的价值

请以 JSON 格式返回：
{
  "work": "与目标人群工作相关的具体场景描述",
  "personal": "与目标人群个人生活相关的具体场景描述",
  "both": "综合研究工作和个人场景对调研目标的价值描述"
}`;
